...
const seedState = {
  ...
  mechanics: [
    { id: "m-1", name: "Lucia Ferrer", phone: "1144556677" },
    { id: "m-2", name: "Mateo Silva", phone: "1166778899" },
  ],
  vehicles: [
    {
      id: "v-1",
...
export default function App() {
  ...
  return (
    <div className="app">
      ...
      {screen === "mechanic" && <MechanicPanel db={db} saveMechanicReview={saveMechanicReview} />}
    </div>
  );
}
  ...
  return (
    <main className="publish">
      ...
      <div className="stickyActions">
        ...
        <button className="primary" onClick={() => publishVehicle({ ...form, photos: form.photos.length ? form.photos : seedImages.slice(0, 1) })}>Publicar automaticamente</button>
      </div>
    </main>
  );
}
...
function MechanicPanel({ db, saveMechanicReview }) {
  const [mechanicId, setMechanicId] = useState(db.mechanics[0]?.id || "");
  const assigned = db.vehicles.filter((v) => v.active && v.assignedMechanicId === mechanicId);
  return (
    <main className="mechanic">
      <section className="panelHeader">
        <div><p className="eyebrow">Panel mecanico</p><h2>Revisiones asignadas</h2></div>
...
function MechanicReview({ vehicle, saveMechanicReview }) {
  const [review, setReview] = useState({ motor: 8, caja: 8, estructura: 8, frenos: 8, electronica: 8, interior: 8, observaciones: "", estado: "aprobado" });
  return (
